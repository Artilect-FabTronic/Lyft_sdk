...
def export_scene_pointcloud(lyftd: lyftdataset,
                            out_path: str,
                            scene_token: str,
    ...
    # Get records from DB.
    scene_rec = lyftd.get('scene', scene_token)
    start_sample_rec = lyftd.get('sample', scene_rec['first_sample_token'])
    sd_rec = lyftd.get('sample_data', start_sample_rec['data'][channel])

    # Make list of frames
    cur_sd_rec = sd_rec
    sd_tokens = []
    while cur_sd_rec['next'] != '':
        cur_sd_rec = lyftd.get('sample_data', cur_sd_rec['next'])
    ...
    with open(out_path, 'w') as f:
        ...
        for sd_token in tqdm(sd_tokens):
            ...
            sc_rec = lyftd.get('sample_data', sd_token)
            sample_rec = lyftd.get('sample', sc_rec['sample_token'])
            lidar_token = sd_rec['token']
            lidar_rec = lyftd.get('sample_data', lidar_token)
            pc = LidarPointCloud.from_file(osp.join(lyftd.data_path, lidar_rec['filename']))

            # Get point cloud colors.
            coloring = np.ones((3, pc.points.shape[1])) * -1
            for channel in camera_channels:
                camera_token = sample_rec['data'][channel]
                cam_coloring, cam_mask = pointcloud_color_from_image(lyftd, lidar_token, camera_token)
            ...
            cs_record = lyftd.get('calibrated_sensor', lidar_rec['calibrated_sensor_token'])
            ...
            poserecord = lyftd.get('ego_pose', lidar_rec['ego_pose_token'])
            ...
            if not sd_rec['next'] == "":
                sd_rec = lyftd.get('sample_data', sd_rec['next'])


def pointcloud_color_from_image(lyftd: lyftdataset,
    ...
    pc = LidarPointCloud.from_file(osp.join(lyftd.data_path, pointsensor['filename']))
...
if __name__ == '__main__':
    ...
    lyftd = lyftdataset()
    scene_tokens = [s['token'] for s in lyftd.scene if s['name'] == scene_name]
    assert len(scene_tokens) == 1, 'Error: Invalid scene %s' % scene_name

    export_scene_pointcloud(lyftd, out_path, scene_tokens[0], channel='LIDAR_TOP', verbose=verbose)
